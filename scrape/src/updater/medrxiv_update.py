...
class MedrxivUpdater(DataUpdater):
    ...
    def _create_datapoint(self, raw_data):
        article = ArticleDataPoint(doi=raw_data['rel_doi'], title=raw_data['rel_title'],
                                   abstract=raw_data['rel_abs'], is_preprint=True)
        site = raw_data['rel_site']
        if site == "medrxiv":
            article.paperhost_name = _MEDRXIV_PAPERHOST_NAME
            article.paperhost_url = _MEDRXIV_PAPERHOST_URL
        elif site == "biorxiv":
            article.paperhost_name = _BIORXIV_PAPERHOST_NAME
            article.paperhost_url = _BIORXIV_PAPERHOST_URL
        article.datasource = DataSource.MEDBIORXIV
        article.url = raw_data['rel_link']
        article.published_at = datetime.strptime(raw_data['rel_date'], "%Y-%m-%d").date()
    ...
    def _get_article(self, doi):
        self._get_article_json()
        try:
            return self._create_datapoint(next(x for x in self._article_json if x['rel_doi'] == doi))
