...
class ElsevierUpdater(DataUpdater):
    ...
    def _create_datapoint(self, article_info):
        article = ArticleDataPoint(is_preprint=False)
        ...
        article.paperhost_name = "Elsevier"
    ...
    def _get_all_articles(self):
        ...
        for doi, article_info in self._metadata.items():
            yield self._create_datapoint(article_info)

    def _get_article(self, doi):
        self._load_metadata()
        try:
            return self._create_datapoint(self._metadata[doi])
        except KeyError:
            return None
